...
class TaskFormer(MDApp):
    ...
    def task_content(self, source):
        ...
        if self.source == "user":
            ...
            due_date_str = self.day_month_yr + " " + self.hour_min
            due_date = datetime.strptime(due_date_str, "%d/%m/%Y %H:%M")
            current_date = datetime.now()
            time_diff = due_date - current_date

            if time_diff <= timedelta(self.day):  # Notify if due date is within 2 days
                Snackbar(text=f"Task '{self.task}' is due in less than 1 days!").open()
    ...
    def task_content(self, source):
        ...
        if self.source == "user":
            ...
            if time_diff <= timedelta(days=2):  # Notify if due date is within 2 days
                Snackbar(text=f"Task '{self.task}' is due in less than 2 days!").open()
            if time_diff <= timedelta(hours=24):  # Notify if due date is within 24 hours
                Snackbar(text=f"Task '{self.task}' is due in less than 24 hours!").open()
    ...
    def notes_content(self, cond: object) -> object:
        self.cond = cond
        ...
        if self.cond == "pass":
            ...
            for self.one in self.gdata:
                ...
                self.current_date_time = self.one[1] if len(self.one) > 1 else ""
                self.card2.add_widget(
                    Show_card_Notes(app, self.text, self.current_date_time)
                )
